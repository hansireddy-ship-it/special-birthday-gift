...
import { generatePoem, generateForecast, generateImage, generateCompliment, generateHistoryFact } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [isPoemLoading, setIsPoemLoading] = useState<boolean>(true);
  const [forecast, setForecast] = useState<string>('');
  const [isForecastLoading, setIsForecastLoading] = useState<boolean>(true);
  const [imageUrl, setImageUrl] = useState<string>('');
  const [isImageLoading, setIsImageLoading] = useState<boolean>(true);
  const [historyFact, setHistoryFact] = useState<string>('');
...
You’re my blessing, my peace, my Devi.!!`;
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [poemText, forecastText, generatedImageUrl, historyText] = await Promise.all([
          generatePoem(),
          generateForecast(),
          generateImage(),
          generateHistoryFact()
